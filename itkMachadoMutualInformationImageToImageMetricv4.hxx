 ...
 *
 *=========================================================================*/
#ifndef itkMachadoMutualInformationImageToImageMetricv4_hxx
#define itkMachadoMutualInformationImageToImageMetricv4_hxx
...
MachadoMutualInformationImageToImageMetricv4<TFixedImage,
                                            TMovingImage,
                                            TVirtualImage,
                                            TInternalComputationValueType,
                                            TMetricTraits>::MachadoMutualInformationImageToImageMetricv4()
  ...
  , m_qValue(1.0)
{
  ...
  this->m_DenseGetValueAndDerivativeThreader = MachadoMutualInformationDenseGetValueAndDerivativeThreaderType::New();
  this->m_SparseGetValueAndDerivativeThreader = MachadoMutualInformationSparseGetValueAndDerivativeThreaderType::New();
                                            ...
                                            TMetricTraits>::ComputeResults() const
{
  ...
  for (unsigned int fixedIndex = 0; fixedIndex < this->m_NumberOfHistogramBins; ++fixedIndex)
  {
    ...
    for (unsigned int movingIndex = 0; movingIndex < this->m_NumberOfHistogramBins; ++movingIndex, jointPDFPtr++)
    {
      ...
      sum += std::pow(jointPDFValue,this->m_qValue) / std::pow((fixedImagePDFValue * movingImagePDFValue),(this->m_qValue-1.0));
      ...
      if (this->GetComputeDerivative())
      {
        if (!this->HasLocalSupport())
        {
               ...
               ++parameter, derivPtr++)
          {
            ...
            (*(this->m_DerivativeResult))[parameter] += (*derivPtr) * qpRatio * (this->m_qValue / (this->m_qValue - 1.0));
  ...
  this->m_Value = static_cast<MeasureType>(-1.0 * (1.0 / (1.0 - this->m_qValue)) * (1.0 - sum));
}
